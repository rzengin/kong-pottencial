...
* **Dificuldade 1 e 2 mapeadas (Tokens Legados e Campos Extras na Autenticação):**
  ...
  * 💡 **Estratégia Zero-Atrito (Migração Transparente de Tokens Atuais):** Para garantir que a migração não impacte os consumidores da API e **evitar que os clientes precisem receber novos tokens ou reautenticar**, o Kong Enterprise oferece nativamente duas vias de migração completamente imperceptíveis:
    1. **Importação em Massa (Seed):** Para tokens OAuth2 opacos armazenados no banco legaddo da Sensedia, podemos scriptar a exportação desses hashes e utilizar a *Admin API* do Kong (`POST /consumers/{id}/oauth2`) para injetar os *access_tokens* originais juntamente com seus tempos de expiração (TTL). O aplicativo cliente continuará enviando exatamente o mesmo `Authorization: Bearer` que já usava.
    2. **Validação Stateless (Para JWTs do Hefesto):** Caso os tokens enviados pelos clientes já sejam JWTs assinados pelo Auth Server de vocês (Keycloak/Hefesto), não há necessidade de migrar banco de dados. Apontaremos a diretiva `issuer` do plugin OIDC do Kong para o Hefesto, e o Kong passará a validar a criptografia (assinatura) desses tokens pré-existentes localmente e em tempo real, sem fricção.
